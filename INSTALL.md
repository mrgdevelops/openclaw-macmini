...
> 📌 OpenClaw's `install.sh` (§2.1 Option B) tries to install Node itself, but on Apple Silicon it has friction: Homebrew permissions fail mid-run, and `node@24` gets installed keg-only without PATH wiring. Community consensus: **install brew + node manually first**.
...
### 1.3 Ollama — install, bind to loopback
...
# → must show 127.0.0.1:11434, never 0.0.0.0
```

### 1.4 Pull Gemma 4

> **16 GB RAM → `gemma4:e4b`** (edge variant, 4B effective params). Confirm tag at <https://ollama.com/library/gemma4>.
...
| 32 GB+ | `gemma4:26b-a4b-it-q4_K_M` | MoE, 4B active. Better quality, ~4× memory. |
| 64 GB+ | `gemma4:26b` / `gemma4:31b` | Dense models, top-end quality. |

> ⚠️ **Agentic caveat for `e4b`.** Edge variants may be less reliable emitting structured tool-call JSON under longer ReAct loops. Start with `e4b`; if tool-calls degrade → (a) Modelfile tune (§1.5), (b) upgrade RAM and move to `26b-a4b-it-q4_K_M`.

```bash
# Pull without tag — registers under all aliases (gemma4, gemma4:latest, gemma4:e4b)
$ ollama pull gemma4
...
> 📌 **Why `ollama pull gemma4` and not `gemma4:e4b`?** Both resolve to the same weights today, but `ollama pull gemma4` registers under **all** aliases. `ollama pull gemma4:e4b` registers **only** as `gemma4:e4b` — and OpenClaw's wizard asks for `gemma4`, causing a 404.

### 1.5 Optional — tune for tool-calling if needed (custom Modelfile)
...
Then update `~/.openclaw/openclaw.json`: swap `"gemma4"` → `"gemma4-claw"` in both `models[].id` and `defaultModel`.
...
### 2.1 Install — three options, choose one
...
> ⚠️ If `pnpm -v` returns `command not found`: check `which node` returns `/opt/homebrew/opt/node@24/bin/node`. Fallback: `brew install pnpm`.
...
> ⚠️ **Option B on Apple Silicon:** (1) may fail on first run — Homebrew needs admin privileges, recover with `sudo !!`; (2) `node@24` installed keg-only, PATH may not be wired — §1.2 already handles this.

```bash
# Option C — npm (last resort)
...
> 📌 **If you used Option B (`install.sh`),** onboarding already started. Follow the guidance below, then skip to §2.3.
...
| Token provision | **Generate/store plaintext token** | See ⚠️ below. |
...
| Model | **`gemma4`** (accept default) | Works if §1.4 used `ollama pull gemma4`. |
...
Stealth mode silently drops unsolicited inbound packets — port scans see the Mac as offline.
...
Plug in a dedicated external SSD, enable Time Machine, encrypt the backup. Skip temporarily if you don't have a disk — circle back.
...
- `openclaw skills list` → remove any skill you didn't install.
...
```bash
# weekly-ish — adjust the OpenClaw line to match your install method from §2.1
...
- [x] Dedicated mobile number — prepaid SIM
- [ ] LinkedIn profile
- [ ] X account

### 5.2 Identity files inside the workspace

OpenClaw treats markdown files at the workspace root as **injected prompt files** — concatenated into LLM context before every inference. Free-form Markdown; convention is `####`-headed sections.
...
| `USER.md` | Who you are — role, timezone, preferences. |
| `AGENTS.md` | Operational rules, sub-agent handoff. |
| `HEARTBEAT.md` | Running state the agent self-updates. |

> ⚠️ These files **shape behaviour, not permissions**. Real boundaries live in `~/.openclaw/openclaw.json` (see §3.6).
...
- **Start with no web search** (skip during onboarding) — add later.
- **Brave Search API** when ready: 2,000 free queries/month. `openclaw configure --section web`.
- **Not SearxNG self-hosted** on this box — competes for RAM with Ollama.

> Perplexity gotcha: model id must be literal `"sonar-pro"`, not `"perplexity/sonar-pro"` (returns HTTP 400).

#### Other skills

- **File-system** — built in, confined to workspace per §3.6.
- **Calendar / mail** — depends on identity accounts (§5.1).
- **Telegram / WhatsApp** — once prepaid SIM is registered, OpenClaw can use community skills.
...
- Apply `sshd_config.d/100-hardening.conf` from §3.4.

### 6.3 Optional: Screen sharing over Tailscale

`System Settings → General → Sharing → Screen Sharing: ON`. Connect from Windows via VNC over the Tailscale IP — never the LAN IP.

✅ **Verify:** Mac reachable as `<machine-name>.<tailnet>.ts.net`. `lsof` shows no listener on LAN IP.
...
Then continue from §2.2 (Onboarding).
...
| `Unauthorized` from OpenClaw → Ollama | Empty `apiKey` in config | Set any non-empty string, e.g. `"ollama-local"` |
| Gateway listens on `0.0.0.0` | Wizard default override | Edit config → `host: "127.0.0.1"`, restart |
